...
const createCart = async function (req, res) {
  try {
    ...
    //validating productIdd
    if (!isValid(productId))
      return res
        .status(400)
        .send({ status: false, message: "productId Id is required" });
    ...
    if (presentCart !== null) {
      ...
      //calculating price and quantityd
...
const removeProduct = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(productId))
      return res
        .status(400)
        .send({ status: false, message: "productId  is not a valid id" });

    //validating cartIdd
    if (!isValid(cartId))
      return res
        .status(400)
        .send({ status: false, message: "cartId Id is required" });
    if (!isValidObjectId(cartId))
      return res
        .status(400)
        .send({ status: false, message: "cartId  is not a valid id" });

    //validating removeProductd
    ...
    if (removeProduct !== 0 && removeProduct !== 1)
      return res
        .status(400)
        .send({ status: false, message: "Please provide number among 0 & 1." });

    //checking product is present or not in product modeld
    let getProduct = await productModel.findById({ _id: productId });
    if (getProduct == null)
      return res
        .status(404)
    ...
    //checking cart is present or not in cart modeld
    let presentCart = await cartModel.findOne({ userId: userId });
    if (presentCart == null)
      return res
        .status(404)
        .send({ status: false, message: "Cart Not Found. First create cart." });
    if (cartId.toString() !== presentCart._id.toString())
      return res
        .status(400)
        .send({ status: false, message: "Please provide your cartId." });
    let cartId1 = presentCart._id;

    //checking product is present or not in cartd
    data["items"] = [];
    let index = 0;
    let product = 0;
    let number = 0;
    ...
    if (product === 0)
      return res
        .status(404)
        .send({ status: false, message: "Product Not in cart." });

    //removing complete product from cart:
    if (removeProduct === 0) {
      //calculating total price and itemsd
...
const getCart = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "cart details", data: getData });
  } catch (error) {

...
const createOrder = async function (req, res) {
  try {
    ...
    if (!isValid(cartId))
      return res
        .status(400)
        .send({ status: false, message: "cart Id required" });
    ...
    if (cart.items.length == 0)
      return res
        ...
        .send({
          status: false,
          message: "Cart is empty... First add Product to Cart.",
        });
    cart.totalQuantity = cart.items
      .map((x) => x.quantity)
      .reduce((x, y) => x + y); // map giving array of quantity
...
const updateOrder = async function (req, res) {
  try {
    ...
    if (searchOrder.userId != userId)
      return res
        .status(400)
        .send({
          status: false,
          message: "the order does not belongs to this user",
        });

    if (!status)
      return res
        .status(400)
        .send({ status: false, message: "Provide Order Status" });
    if (!isValidStatus(status))
      return res
        ...
        .send({
          status: false,
          message:
            "status should be among 'pending','completed' and 'canceled' only",
        });

    if (status == "cancled" && searchOrder.cancellable !== true)
      return res
        .status(400)
        .send({ status: false, message: "You can not cancel the order" });

  ...
  try {
    ...
    if (!isValidPrice(data.price))
      return res
        ...
        .send({
          status: false,
          message: `${data.price} is not a valid price. Please provide input in numbers.`,
        });
    ...
    if (data.currencyId.trim() !== "INR")
      return res
        .status(400)
        .send({ status: false, message: "Please provide Indian Currency Id" });
    ...
    if (!isValid(data.style))
      return res
        ...
        .send({
          status: false,
          message: "Please provide style for your product",
        });
    ...
    if (!isValid(data.availableSizes))
      return res
        ...
        .send({
          status: false,
          message: "Please provide available size for your product1",
        });
  ...
  try {
    ...
    if (Object.keys(requestQuery).length > 0) {
      if (size) {
        ...
        if (
          size1.map((x) => isValidSize(x)).filter((x) => x === false).length !==
          0
        )
          return res
            .status(400)
      ...
      if (priceSort) {
        if (priceSort != 1 && priceSort != -1)
          return res
            ...
            .send({
              status: false,
              message: "priceSort should be among 1 and -1.",
            });
      }

      if (priceGreaterThan && priceLessThan) {
        filterQuery.price = { $gt: priceGreaterThan, $lt: priceLessThan };
      }
      if (priceGreaterThan && !priceLessThan) {
    ...
    if (findProducts.length == 0)
      return res
        ...
        .send({
          status: false,
          message: "products not found or may be deleted",
        });

    return res
      .status(200)
      .send({ status: true, message: "products list", data: findProducts });
  } catch (err) {
...
const getProductById = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Product Details", data: product });
  } catch (error) {
...
const updateProductDetails = async function (req, res) {
  try {
    ...
    if (title == "") {
      ...
      if (await productModel.findOne({ title: updateData.title.toUpperCase() }))
        return res
          .status(400)
    ...
    if (availableSizes == "") {
      return res
        .status(400)
        .send({ status: false, message: "availableSizes is not valid" });
      ...
      const availableSizes = updateData.availableSizes
        .toUpperCase()
        .trim()
        .split(",")
    ...
    if (installments == "") {
      return res
        .status(400)
        .send({ status: false, message: "installments is not valid" });
...
const deleteProduct = async function (req, res) {
  try {
    ...
    if (!checkId)
      return res
        .status(400)
        .send({ status: false, msg: " This productId does not exist" });

...
const registerUser = async function (req, res) {
  try {
    ...
    if (!isValid(fname))
      return res
        .status(400)
        .send({ status: false, msg: "Name is a mandatory field" });
    ...
    if (!isValid(lname))
      return res
        .status(400)
        .send({ status: false, msg: "Last Name is a mandatory field" });
    ...
    if (!isValid(phone))
      return res
        .status(400)
        .send({ status: false, msg: "Phone number is a mandatory field" });
    if (!isValidMobile(phone))
      return res
        .status(400)
        .send({ status: false, msg: `${phone} number is not a valid` });
    let isUniquePhone = await userModel.findOne({ phone: phone });
    if (isUniquePhone)
      return res
        .status(400)
        .send({ status: false, msg: `${phone} number is already exist` });
...
const userLogin = async function (req, res) {
  try {
    ...
    if (!user) {
      res
        .status(404)
        .send({ status: false, message: `${email} related user unavailable` });
      return;
    }
    const isLogin = await bcrypt
      .compare(password, user.password)
      .catch((e) => false);
    if (!isLogin) {
      res
        .status(401)
        .send({ status: false, message: `wrong email address or password` });
      return;
    }
